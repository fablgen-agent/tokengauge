      ...
      <main>
        ...
        {account ? (
          <section className="lab-shell section-pad">
            <aside className="lab-note"><h2>{supportedTips.length} controlled recipes</h2><p>Each test makes two model requests against your connected ChatGPT plan. Token counts do not represent an API invoice.</p><p>TokenGauge stores the model, strategy label, and usage totals. It does not store either prompt or either output.</p><p>Other catalogue cards are labeled as guided protocols until they have a real adapter. Judge quality before declaring a winner.</p><AccountPanel compact /></aside>
            <LabWorkbench strategies={supportedTips.map(({ id, title, action }) => ({ id, title, action }))} />
          </section>
        ) : <section className="gate-card"><h2>Connect ChatGPT to run a paired test.</h2><p>{supportedTips.length} controlled request-setting recipes are free. TokenGauge uses your connected ChatGPT plan and stores usage totals—not prompts or outputs.</p><AccountPanel /></section>}
      </main>
    </div>
  );
}
