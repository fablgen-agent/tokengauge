...
export default async function WorkPage({ searchParams }: { searchParams: Promise<{ service?: string }> }) {
  ...
  const jsonLd = {
    ...
    itemListElement: services.map((service, index) => ({ "@type": "ListItem", position: index + 1, name: service.label, description: service.price })),
  };
  ...
  return <div className="work-page">
    ...
    <main>
      ...
      <section className="work-scope section-pad"><div className="section-heading split-heading"><div><span className="eyebrow">EXISTING OFFERS ONLY</span><h2>Small, reviewable,<br />and priced in writing.</h2></div><p>This desk does not expand a request to fit a price. If the public context does not match one bounded lane, the reply says so before private access or payment.</p></div><div className="work-scope-grid">{services.map((service) => <article key={service.id}><span>{service.price}</span><h3>{service.label}</h3></article>)}</div></section>
