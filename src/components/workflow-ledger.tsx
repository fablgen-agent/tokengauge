    ...
    <div className="ledger-tool">
      ...
      <section className="ledger-results" aria-live="polite">
        ...
        <div className="ledger-attribution-grid">
          <div><span className="eyebrow">BY WORKFLOW</span>{ledger.rows.map((row) => <article key={row.id}><div><strong>{row.workflow}</strong><small>{row.project} · {row.providerLabel} {row.modelLabel}</small>{row.cacheTreatment === "ordinary-input-fallback" ? <small>Cache price unavailable · ordinary-input fallback applied</small> : row.cacheTreatment === "published-rate" ? <small>Published cache-read rate applied</small> : null}</div><div><b>{formatUsd(row.modeledCostUsd)}</b><small>{row.costPerAcceptedAnswerUsd === null ? "no accepted answers" : `${formatSmallUsd(row.costPerAcceptedAnswerUsd)} / accepted`}</small></div></article>)}</div>
        ...
        <p className="ledger-caveat">{ledger.cacheFallbackRowCount > 0 ? <><strong>{ledger.cacheFallbackRowCount} {ledger.cacheFallbackRowCount === 1 ? "row uses" : "rows use"} ordinary-input pricing because no separate cache-read rate was published.</strong> This is a conservative placeholder, not the provider&apos;s cache price. </> : null}These are rate-card estimates, not an invoice. Map each aggregate to the price band that covered its individual requests. Tools, cache writes/storage, media, regional or priority uplifts, taxes, credits, and provider rounding remain outside rows that do not explicitly represent them.</p>
